...
for model in models:
    ...
    tweet_dict = {row[0]: row[1] for row in tweets}
    ...
    if response.status_code == 200:
        ...
        for tweet_id, result in sentiment_data.items():
            ...
            if existing_record:
                ...
                cur.execute("UPDATE sentiment_results SET sentiment_result = %s, sentiment_score = %s, created_date = %s WHERE id = %s", (sentiment, sentiment_score, created_date, existing_record[0]))
            else:
                # Insert a new record
                cur.execute("INSERT INTO sentiment_results (tweet_id, model_name_id, sentiment_result, sentiment_score, created_date) VALUES (%s, %s, %s, %s, %s)", (tweet_id, model_id, sentiment, sentiment_score, created_date))
